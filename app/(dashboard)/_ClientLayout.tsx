...
export default function DashboardLayout({
  children,
  recalculando: recalculandoInicial = false,
}: {
  ...
  const [recalculando, setRecalculando] = useState(recalculandoInicial);
  ...
  // Recálculo automático: se ejecuta una sola vez si el servidor indicó que es necesario
  useEffect(() => {
    if (!recalculandoInicial || yaEjecutado.current) return;
    yaEjecutado.current = true;

    fetch('/api/recalcular', { method: 'POST' })
      .then(() => {
        // Refrescar datos del servidor con información ya actualizada
        router.refresh();
        setRecalculando(false);
      })
      .catch(() => {
        // Si falla, continuar normalmente sin bloquear
        setRecalculando(false);
      });
  }, [recalculandoInicial, router]);
  ...
  // Pantalla de carga mientras se recalculan los estatus del día
  if (recalculando) {
    return (
        ...
        <div className="flex flex-col items-center gap-5 text-center px-6">
          ...
          <div>
            <p className="text-primary font-display font-bold text-lg">
              Recalculando información del día
            </p>
            <p className="text-text-muted text-sm mt-1">
              Esto ocurre una sola vez al día y toma unos segundos…
            </p>
          </div>
        </div>
      </div>
